...
if (isset($_POST['btn_add'])) {
  ...
  // Use NOW() for datetimeRecorded to capture the current date and time.
  $query = mysqli_query($con, "INSERT INTO tblblotter (yearRecorded, dateRecorded, datetimeRecorded, complainant, cage, caddress, ccontact, personToComplain, page, paddress, pcontact, complaint, actionTaken, sStatus, locationOfIncidence, recordedby) 
        VALUES ('$year', '$date', NOW(), '$txt_cname', '$txt_cage', '$txt_cadd', '$txt_ccontact', '$txt_pname', '$txt_page', '$txt_padd', '$txt_pcontact', '$txt_complaint', '$ddl_acttaken', '$ddl_stat', '$txt_location', '" . $_SESSION['fname'] . " " . $_SESSION['lname'] . "')") or die('Error: ' . mysqli_error($con));
