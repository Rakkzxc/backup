...
if (isset($_POST['btn_add'])) {
  ...
  if (isset($_SESSION['role'])) {
    $action = 'added officer named ' . $input_name;
  ...
  $stmt = mysqli_prepare($con, "SELECT * FROM tbladdofficeroftheday WHERE name = ?");
  ...
  if ($ct == 0) {
    $query = mysqli_prepare($con, "INSERT INTO tbladdofficeroftheday (name, position, created, added_by) VALUES (?, ?, ?, ?)");
    mysqli_stmt_bind_param($query, 'ssss', $input_name, $input_position, $input_created, $input_added_by);
...
if (isset($_POST['btn_delete'])) {
  if (isset($_POST['chk_delete'])) {
    foreach ($_POST['chk_delete'] as $value) {
      $delete_query = mysqli_query($con, "DELETE FROM tbladdofficeroftheday WHERE id = '$value' ") or die('Error: ' . mysqli_error($con));
